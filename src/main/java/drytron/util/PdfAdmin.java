...
public class PdfAdmin {
    ...
    public static void gerar(String dir) {
        try {
            ...
            PdfWriter.getInstance(document, new FileOutputStream(dir + "\\AdminRelatorio.pdf"));
            document.open();
    ...
    private static void addMetaDado(Document document) {
        document.addTitle("Drytron - Administrador");
        document.addSubject("Area: Administrador");
        document.addKeywords("Drytron, Administrador");
        document.addAuthor("Drytron - Sistema de gerenciamento");
        document.addCreator("Drytron - Sistema de gerenciamento");
    }

    private static void addContent(Document document) throws DocumentException {
        Anchor anchor = new Anchor("Area Administrador", catFont);
        anchor.setName("Area Administrador");

        Chapter catPart = new Chapter(new Paragraph(anchor), 1);

        Paragraph prefacio = new Paragraph();
        addLinhaVazia(prefacio, 1);
        prefacio.add(new Paragraph("Administrador", catFont));
        addLinhaVazia(prefacio, 1);
        ...
        Paragraph subPara = new Paragraph("Tabela Administrador \n\n", subFont);
    ...
    private static void criarTabela(Section subCatPart) throws BadElementException {
        ...
        c1 = new PdfPCell(new Paragraph("CARGO", new Font(Font.FontFamily.TIMES_ROMAN, 6, Font.BOLD)));
        c1.setHorizontalAlignment(Element.ALIGN_CENTER);
        table.addCell(c1);
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            table.addCell(new Phrase(Element.ALIGN_LEFT, list.get(i).getEndFun().getLocalidade(),
                    new Font(Font.FontFamily.TIMES_ROMAN, fonteDados, Font.BOLD)));

            table.addCell(new Phrase(Element.ALIGN_LEFT, list.get(i).getEndFun().getBairro(),
                    new Font(Font.FontFamily.TIMES_ROMAN, fonteDados, Font.BOLD)));

            table.addCell(new Phrase(Element.ALIGN_LEFT, list.get(i).getEndFun().getUf(),
                    new Font(Font.FontFamily.TIMES_ROMAN, fonteDados, Font.BOLD)));
        }
